...
internal class EpisodeTest {
    ...
    fun create() {
      ...
      assertThat(episode.characterFocus()).containsExactly("ted-lasso", "rebecca-welton", "coach-beard")
      ...
      assertThat(episode.synopsis()).isEqualTo("American football coach Ted Lasso is hired to manage AFC Richmond, a struggling English Premier League team.")
      assertThat(episode.tedWisdom()).isEqualTo("Taking on a challenge is a lot like riding a horse. If you're comfortable while you're doing it, you're probably doing it wrong.")
      assertThat(episode.title()).isEqualTo("Pilot")
      assertThat(episode.writer()).isEqualTo("Jason Sudeikis, Bill Lawrence, Brendan Hunt, Joe Kelly")
      assertThat(episode.biscuitsWithBossMoment()).isEqualTo("Ted brings Rebecca homemade biscuits for the first time.")
      assertThat(episode.memorableMoments()).containsExactly("Ted's first press conference", "The BELIEVE sign goes up", "Ted tastes his first 'garbage water' (English tea)")
    ...
    fun roundtrip() {
      ...
      val roundtrippedEpisode = jsonMapper.readValue(jsonMapper.writeValueAsString(episode), jacksonTypeRef<Episode>())
