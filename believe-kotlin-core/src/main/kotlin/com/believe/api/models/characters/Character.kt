...
class Character @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    @ExcludeMissing
    fun _salaryGbp(): JsonField<String> = salaryGbp
    ...
    class Builder internal constructor() {
        ...
        internal fun from(character: Character) =
            apply {
                id = character.id
        ...
        /** Emotional intelligence stats */
        fun emotionalStats(emotionalStats: EmotionalStats) = emotionalStats(JsonField.of(emotionalStats))
        ...
        /** Key personality traits */
        fun personalityTraits(personalityTraits: List<String>) = personalityTraits(JsonField.of(personalityTraits))
         ...
         *
         * You should usually call [Builder.salaryGbp] with a well-typed [String] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun salaryGbp(salaryGbp: JsonField<String>) =
            apply {
                this.salaryGbp = salaryGbp
            }

        /** Memorable quotes from this character */
        fun signatureQuotes(signatureQuotes: List<String>) = signatureQuotes(JsonField.of(signatureQuotes))
     ...
     */
    internal fun validity(): Int = (if (id.asKnown() == null) 0 else 1) + (if (background.asKnown() == null) 0 else 1) + (emotionalStats.asKnown()?.validity() ?: 0) + (if (name.asKnown() == null) 0 else 1) + (personalityTraits.asKnown()?.size ?: 0) + (role.asKnown()?.validity() ?: 0) + (if (dateOfBirth.asKnown() == null) 0 else 1) + (if (email.asKnown() == null) 0 else 1) + (growthArcs.asKnown()?.sumOf { it.validity().toInt() } ?: 0) + (if (heightMeters.asKnown() == null) 0 else 1) + (if (profileImageUrl.asKnown() == null) 0 else 1) + (if (salaryGbp.asKnown() == null) 0 else 1) + (signatureQuotes.asKnown()?.size ?: 0) + (if (teamId.asKnown() == null) 0 else 1)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is Character && id == other.id && background == other.background && emotionalStats == other.emotionalStats && name == other.name && personalityTraits == other.personalityTraits && role == other.role && dateOfBirth == other.dateOfBirth && email == other.email && growthArcs == other.growthArcs && heightMeters == other.heightMeters && profileImageUrl == other.profileImageUrl && salaryGbp == other.salaryGbp && signatureQuotes == other.signatureQuotes && teamId == other.teamId && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, background, emotionalStats, name, personalityTraits, role, dateOfBirth, email, growthArcs, heightMeters, profileImageUrl, salaryGbp, signatureQuotes, teamId, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "Character{id=$id, background=$background, emotionalStats=$emotionalStats, name=$name, personalityTraits=$personalityTraits, role=$role, dateOfBirth=$dateOfBirth, email=$email, growthArcs=$growthArcs, heightMeters=$heightMeters, profileImageUrl=$profileImageUrl, salaryGbp=$salaryGbp, signatureQuotes=$signatureQuotes, teamId=$teamId, additionalProperties=$additionalProperties}"
}
