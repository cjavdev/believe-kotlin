...
class CharacterCreateParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(characterCreateParams: CharacterCreateParams) =
            apply {
        ...
        fun salaryGbp(double: Double) =
            apply {
                body.salaryGbp(double)
            }
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        internal fun validity(): Int = (if (background.asKnown() == null) 0 else 1) + (emotionalStats.asKnown()?.validity() ?: 0) + (if (name.asKnown() == null) 0 else 1) + (personalityTraits.asKnown()?.size ?: 0) + (role.asKnown()?.validity() ?: 0) + (if (dateOfBirth.asKnown() == null) 0 else 1) + (if (email.asKnown() == null) 0 else 1) + (growthArcs.asKnown()?.sumOf { it.validity().toInt() } ?: 0) + (if (heightMeters.asKnown() == null) 0 else 1) + (if (profileImageUrl.asKnown() == null) 0 else 1) + (salaryGbp.asKnown()?.validity() ?: 0) + (signatureQuotes.asKnown()?.size ?: 0) + (if (teamId.asKnown() == null) 0 else 1)
        ...
        private val hashCode: Int by lazy { Objects.hash(background, emotionalStats, name, personalityTraits, role, dateOfBirth, email, growthArcs, heightMeters, profileImageUrl, salaryGbp, signatureQuotes, teamId, additionalProperties) }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is CharacterCreateParams && body == other.body && additionalHeaders == other.additionalHeaders && additionalQueryParams == other.additionalQueryParams
    }

    override fun hashCode(): Int = Objects.hash(body, additionalHeaders, additionalQueryParams)

    override fun toString() = "CharacterCreateParams{body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
