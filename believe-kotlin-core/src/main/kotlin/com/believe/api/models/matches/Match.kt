...
class Match @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("ticket_revenue_gbp") @ExcludeMissing ticketRevenueGbp: JsonField<String> = JsonMissing.of(),
    ...
    class Builder internal constructor() {
        ...
        /** Total ticket revenue in GBP */
        fun ticketRevenueGbp(ticketRevenueGbp: String?) = ticketRevenueGbp(JsonField.ofNullable(ticketRevenueGbp))

        /**
         * Sets [Builder.ticketRevenueGbp] to an arbitrary JSON value.
         *
         * You should usually call [Builder.ticketRevenueGbp] with a well-typed [String] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun ticketRevenueGbp(ticketRevenueGbp: JsonField<String>) =
            apply {
                this.ticketRevenueGbp = ticketRevenueGbp
            }

        /** Key moments that changed the match */
        fun turningPoints(turningPoints: List<TurningPoint>) = turningPoints(JsonField.of(turningPoints))
     ...
     */
    internal fun validity(): Int = (if (id.asKnown() == null) 0 else 1) + (if (awayTeamId.asKnown() == null) 0 else 1) + (if (date.asKnown() == null) 0 else 1) + (if (homeTeamId.asKnown() == null) 0 else 1) + (matchType.asKnown()?.validity() ?: 0) + (if (attendance.asKnown() == null) 0 else 1) + (if (awayScore.asKnown() == null) 0 else 1) + (if (episodeId.asKnown() == null) 0 else 1) + (if (homeScore.asKnown() == null) 0 else 1) + (if (lessonLearned.asKnown() == null) 0 else 1) + (if (possessionPercentage.asKnown() == null) 0 else 1) + (result.asKnown()?.validity() ?: 0) + (if (tedHalftimeSpeech.asKnown() == null) 0 else 1) + (if (ticketRevenueGbp.asKnown() == null) 0 else 1) + (turningPoints.asKnown()?.sumOf { it.validity().toInt() } ?: 0) + (if (weatherTempCelsius.asKnown() == null) 0 else 1)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is Match && id == other.id && awayTeamId == other.awayTeamId && date == other.date && homeTeamId == other.homeTeamId && matchType == other.matchType && attendance == other.attendance && awayScore == other.awayScore && episodeId == other.episodeId && homeScore == other.homeScore && lessonLearned == other.lessonLearned && possessionPercentage == other.possessionPercentage && result == other.result && tedHalftimeSpeech == other.tedHalftimeSpeech && ticketRevenueGbp == other.ticketRevenueGbp && turningPoints == other.turningPoints && weatherTempCelsius == other.weatherTempCelsius && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, awayTeamId, date, homeTeamId, matchType, attendance, awayScore, episodeId, homeScore, lessonLearned, possessionPercentage, result, tedHalftimeSpeech, ticketRevenueGbp, turningPoints, weatherTempCelsius, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "Match{id=$id, awayTeamId=$awayTeamId, date=$date, homeTeamId=$homeTeamId, matchType=$matchType, attendance=$attendance, awayScore=$awayScore, episodeId=$episodeId, homeScore=$homeScore, lessonLearned=$lessonLearned, possessionPercentage=$possessionPercentage, result=$result, tedHalftimeSpeech=$tedHalftimeSpeech, ticketRevenueGbp=$ticketRevenueGbp, turningPoints=$turningPoints, weatherTempCelsius=$weatherTempCelsius, additionalProperties=$additionalProperties}"
}
