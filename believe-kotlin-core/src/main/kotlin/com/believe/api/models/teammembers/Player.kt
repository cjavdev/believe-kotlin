...
class Player @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is Player && id == other.id && characterId == other.characterId && jerseyNumber == other.jerseyNumber && position == other.position && teamId == other.teamId && yearsWithTeam == other.yearsWithTeam && assists == other.assists && goalsScored == other.goalsScored && isCaptain == other.isCaptain && memberType == other.memberType && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, characterId, jerseyNumber, position, teamId, yearsWithTeam, assists, goalsScored, isCaptain, memberType, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "Player{id=$id, characterId=$characterId, jerseyNumber=$jerseyNumber, position=$position, teamId=$teamId, yearsWithTeam=$yearsWithTeam, assists=$assists, goalsScored=$goalsScored, isCaptain=$isCaptain, memberType=$memberType, additionalProperties=$additionalProperties}"
}
