...
class MatchUpdateParams private constructor(
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        internal fun validity(): Int = (if (attendance.asKnown() == null) 0 else 1) + (if (awayScore.asKnown() == null) 0 else 1) + (if (awayTeamId.asKnown() == null) 0 else 1) + (if (date.asKnown() == null) 0 else 1) + (if (episodeId.asKnown() == null) 0 else 1) + (if (homeScore.asKnown() == null) 0 else 1) + (if (homeTeamId.asKnown() == null) 0 else 1) + (if (lessonLearned.asKnown() == null) 0 else 1) + (matchType.asKnown()?.validity() ?: 0) + (if (possessionPercentage.asKnown() == null) 0 else 1) + (result.asKnown()?.validity() ?: 0) + (if (tedHalftimeSpeech.asKnown() == null) 0 else 1) + (ticketRevenueGbp.asKnown()?.validity() ?: 0) + (turningPoints.asKnown()?.sumOf { it.validity().toInt() } ?: 0) + (if (weatherTempCelsius.asKnown() == null) 0 else 1)
