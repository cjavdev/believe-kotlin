...
class EquipmentManager @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    internal fun validity(): Int = (if (id.asKnown() == null) 0 else 1) + (if (characterId.asKnown() == null) 0 else 1) + (if (teamId.asKnown() == null) 0 else 1) + (if (yearsWithTeam.asKnown() == null) 0 else 1) + (if (isHeadKitman.asKnown() == null) 0 else 1) + (memberType.asKnown()?.validity() ?: 0) + (responsibilities.asKnown()?.size ?: 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is EquipmentManager && id == other.id && characterId == other.characterId && teamId == other.teamId && yearsWithTeam == other.yearsWithTeam && isHeadKitman == other.isHeadKitman && memberType == other.memberType && responsibilities == other.responsibilities && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, characterId, teamId, yearsWithTeam, isHeadKitman, memberType, responsibilities, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "EquipmentManager{id=$id, characterId=$characterId, teamId=$teamId, yearsWithTeam=$yearsWithTeam, isHeadKitman=$isHeadKitman, memberType=$memberType, responsibilities=$responsibilities, additionalProperties=$additionalProperties}"
}
