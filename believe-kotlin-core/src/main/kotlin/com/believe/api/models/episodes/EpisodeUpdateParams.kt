...
class EpisodeUpdateParams private constructor(
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        internal fun validity(): Int = (if (airDate.asKnown() == null) 0 else 1) + (if (biscuitsWithBossMoment.asKnown() == null) 0 else 1) + (characterFocus.asKnown()?.size ?: 0) + (if (director.asKnown() == null) 0 else 1) + (if (episodeNumber.asKnown() == null) 0 else 1) + (if (mainTheme.asKnown() == null) 0 else 1) + (memorableMoments.asKnown()?.size ?: 0) + (if (runtimeMinutes.asKnown() == null) 0 else 1) + (if (season.asKnown() == null) 0 else 1) + (if (synopsis.asKnown() == null) 0 else 1) + (if (tedWisdom.asKnown() == null) 0 else 1) + (if (title.asKnown() == null) 0 else 1) + (if (usViewersMillions.asKnown() == null) 0 else 1) + (if (viewerRating.asKnown() == null) 0 else 1) + (if (writer.asKnown() == null) 0 else 1)
