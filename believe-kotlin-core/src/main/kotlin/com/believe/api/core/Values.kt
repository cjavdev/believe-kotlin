...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw BelieveInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw BelieveInvalidDataException("`$name` is not set")
            is JsonNull -> throw BelieveInvalidDataException("`$name` is null")
            else -> throw BelieveInvalidDataException("`$name` is invalid, received $this")
        }
