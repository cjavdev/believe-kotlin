...
class BelieveClientAsyncImpl(
    ...
    private val withRawResponse: BelieveClientAsync.WithRawResponse by lazy { WithRawResponseImpl(clientOptions) }

    private val characters: CharacterServiceAsync by lazy { CharacterServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val teams: TeamServiceAsync by lazy { TeamServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val matches: MatchServiceAsync by lazy { MatchServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val episodes: EpisodeServiceAsync by lazy { EpisodeServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val quotes: QuoteServiceAsync by lazy { QuoteServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val believe: BelieveServiceAsync by lazy { BelieveServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val conflicts: ConflictServiceAsync by lazy { ConflictServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val reframe: ReframeServiceAsync by lazy { ReframeServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val press: PressServiceAsync by lazy { PressServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val coaching: CoachingServiceAsync by lazy { CoachingServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val biscuits: BiscuitServiceAsync by lazy { BiscuitServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val pepTalk: PepTalkServiceAsync by lazy { PepTalkServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val stream: StreamServiceAsync by lazy { StreamServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val teamMembers: TeamMemberServiceAsync by lazy { TeamMemberServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val webhooks: WebhookServiceAsync by lazy { WebhookServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val ticketSales: TicketSaleServiceAsync by lazy { TicketSaleServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val health: HealthServiceAsync by lazy { HealthServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val version: VersionServiceAsync by lazy { VersionServiceAsyncImpl(clientOptionsWithUserAgent) }

    private val client: ClientServiceAsync by lazy { ClientServiceAsyncImpl(clientOptionsWithUserAgent) }

    override fun sync(): BelieveClient = sync

    override fun withRawResponse(): BelieveClientAsync.WithRawResponse = withRawResponse

    override fun withOptions(modifier: (ClientOptions.Builder) -> Unit): BelieveClientAsync = BelieveClientAsyncImpl(clientOptions.toBuilder().apply(modifier).build())
    ...
    override suspend fun getWelcome(params: ClientGetWelcomeParams, requestOptions: RequestOptions): ClientGetWelcomeResponse =
        // get /
    ...
    class WithRawResponseImpl internal constructor(
        ...
        private val characters: CharacterServiceAsync.WithRawResponse by lazy { CharacterServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val teams: TeamServiceAsync.WithRawResponse by lazy { TeamServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val matches: MatchServiceAsync.WithRawResponse by lazy { MatchServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val episodes: EpisodeServiceAsync.WithRawResponse by lazy { EpisodeServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val quotes: QuoteServiceAsync.WithRawResponse by lazy { QuoteServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val believe: BelieveServiceAsync.WithRawResponse by lazy { BelieveServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val conflicts: ConflictServiceAsync.WithRawResponse by lazy { ConflictServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val reframe: ReframeServiceAsync.WithRawResponse by lazy { ReframeServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val press: PressServiceAsync.WithRawResponse by lazy { PressServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val coaching: CoachingServiceAsync.WithRawResponse by lazy { CoachingServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val biscuits: BiscuitServiceAsync.WithRawResponse by lazy { BiscuitServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val pepTalk: PepTalkServiceAsync.WithRawResponse by lazy { PepTalkServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val stream: StreamServiceAsync.WithRawResponse by lazy { StreamServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val teamMembers: TeamMemberServiceAsync.WithRawResponse by lazy { TeamMemberServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val webhooks: WebhookServiceAsync.WithRawResponse by lazy { WebhookServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val ticketSales: TicketSaleServiceAsync.WithRawResponse by lazy { TicketSaleServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val health: HealthServiceAsync.WithRawResponse by lazy { HealthServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val version: VersionServiceAsync.WithRawResponse by lazy { VersionServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        private val client: ClientServiceAsync.WithRawResponse by lazy { ClientServiceAsyncImpl.WithRawResponseImpl(clientOptions) }

        override fun withOptions(modifier: (ClientOptions.Builder) -> Unit): BelieveClientAsync.WithRawResponse = BelieveClientAsyncImpl.WithRawResponseImpl(clientOptions.toBuilder().apply(modifier).build())
        ...
        override suspend fun getWelcome(params: ClientGetWelcomeParams, requestOptions: RequestOptions): HttpResponseFor<ClientGetWelcomeResponse> {
