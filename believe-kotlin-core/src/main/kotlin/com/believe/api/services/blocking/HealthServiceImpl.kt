...
class HealthServiceImpl internal constructor(
    private val clientOptions: ClientOptions,

) : HealthService {

    private val withRawResponse: HealthService.WithRawResponse by lazy { WithRawResponseImpl(clientOptions) }

    override fun withRawResponse(): HealthService.WithRawResponse = withRawResponse

    override fun withOptions(modifier: (ClientOptions.Builder) -> Unit): HealthService = HealthServiceImpl(clientOptions.toBuilder().apply(modifier).build())

    override fun check(params: HealthCheckParams, requestOptions: RequestOptions): HealthCheckResponse =
        // get /health
    ...
    class WithRawResponseImpl internal constructor(
        ...
        override fun withOptions(modifier: (ClientOptions.Builder) -> Unit): HealthService.WithRawResponse = HealthServiceImpl.WithRawResponseImpl(clientOptions.toBuilder().apply(modifier).build())

        private val checkHandler: Handler<HealthCheckResponse> = jsonHandler<HealthCheckResponse>(clientOptions.jsonMapper)

        override fun check(params: HealthCheckParams, requestOptions: RequestOptions): HttpResponseFor<HealthCheckResponse> {
